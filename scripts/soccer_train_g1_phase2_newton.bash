...
# Resume from Newton phase-1 checkpoint with higher ball-velocity weight.
# Override checkpoint: sbatch --export=ALL,PHASE1_MODEL=output/soccer_g1_newton_phase1/model_xxxxx.pt scripts/soccer_train_g1_phase2_newton.bash
...
# Resolve PHASE1_MODEL: explicit > latest Newton phase 1 final model.pt > latest Newton phase 1 int_models checkpoint.
if [ -z "${PHASE1_MODEL:-}" ]; then
    PHASE1_FINAL=$(ls -1 output/soccer_g1_newton_phase1*/model.pt 2>/dev/null | sort | tail -n 1 || true)
    PHASE1_INT=$(ls -1 output/soccer_g1_newton_phase1*/int_models/model_*.pt 2>/dev/null | sort | tail -n 1 || true)
    if [ -n "${PHASE1_FINAL}" ]; then
        PHASE1_MODEL="${PHASE1_FINAL}"
    elif [ -n "${PHASE1_INT}" ]; then
        PHASE1_MODEL="${PHASE1_INT}"
    else
        echo "ERROR: no phase 1 checkpoint found under output/soccer_g1_newton_phase1*/; set PHASE1_MODEL explicitly." >&2
        exit 1
    fi
fi
echo "Phase 1 source model: ${PHASE1_MODEL}"

RUN_PREFIX=output/soccer_g1_newton_phase2
FRESH="${FRESH:-0}"
if [ "${FRESH}" = "1" ]; then
    LATEST_INT=""
    LATEST_FINAL=""
...
python -u mimickit/run.py --mode train --num_envs 2048 --engine_config data/engines/newton_engine.yaml --env_config data/envs/amp_soccer_g1_env_phase2.yaml --agent_config data/agents/amp_task_g1_agent_phase2.yaml --model_file "${RESUME_MODEL}" --visualize false --out_dir "${OUT_DIR}" --logger tb --save_int_models true
