...
{\tt\small ehalicki@student.ubc.ca}
\and
Jay Janaskar\\
Norwegian University of Science and Technology, ETH Z\"urich\\
...
DeepMimic~\cite{2018-TOG-deepMimic} reproduce reference clips faithfully but
...
prior work, DribbleMaster~\cite{wang2025dribble}, achieves strong sim-to-real
...
\begin{enumerate}
    ...
    \item An open-source implementation on top of MimicKit~\cite{MimicKitPeng2025}
...
DeepMimic~\cite{2018-TOG-deepMimic} and follow-ups track reference clips with a
...
DribbleMaster~\cite{wang2025dribble} learns RL humanoid dribbling with sim-to-real
...
We build on MimicKit~\cite{MimicKitPeng2025}, an open-source imitation-plus-RL
...
$2000$) than the AMP policy. For the same training budget, this translates into
significantly better task tracking: as \cref{tab:amp-ablation} shows, the no-AMP
baseline roughly halves both the speed error ($0.390$ vs.\ $0.771$\,m/s) and the
angle error ($9.02^\circ$ vs.\ $18.54^\circ$) and stays closer to the ball
($0.453$ vs.\ $0.645$\,m), beating AMP on all three metrics. This is expected:
freed from having to match the style prior, the policy can spend its entire
capacity and training budget on the task. The resulting gait, however, is very
unhuman-like.
...
Model & Speed err.\ & Angle err.\ & Char--ball \\
      & (m/s)       & (deg)       & dist.\ (m) \\
\midrule
No AMP        & \textbf{0.390} & \textbf{9.02} & \textbf{0.453} \\
AMP (ours)    & 0.771 & 18.54 & 0.645 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Per-metric comparison of the AMP and no-AMP policies on the Stage-2
dribbling task. For an equal training budget the no-AMP baseline is
significantly better on every metric, since it is not constrained to a natural
gait and can devote all of its capacity to the task.}
...
stage. We also find a clear tension between task performance and style: for an
equal training budget, the no-AMP baseline is substantially better at the task,
roughly halving both speed and angle error and staying closer to the ball
(\cref{tab:amp-ablation}). The style discriminator pulls the policy toward a
clean walking gait, which competes with the contact-rich motion dribbling
requires, so the style reward buys natural locomotion at a measurable cost in
tracking accuracy.
...
A central direction is to better balance the task and style rewards so that the
naturalness of AMP can be recovered without sacrificing the task performance the
no-AMP baseline demonstrates is achievable. Rather than a fixed
$(w_\text{task}, w_\text{style})$ split, future work could anneal the weights
over training: start style-heavy so the agent first acquires a natural,
well-regularized gait, then gradually shift weight toward the task so it can
specialize that gait for the contact-rich demands of dribbling. The style prior
could also be adapted online to hold the discriminator near a target accuracy.
Retargeting motion clips from real soccer players would further give the
discriminator a style closer to the contact-rich gait the task needs, likely
shrinking the gap. Other promising directions include a vision-based,
end-to-end policy that replaces the oracle ball position with visual input
(currently limited by MimicKit's lack of support for CNN/Transformer policies),
sim-to-real transfer to real G1 hardware, and a controlled baseline against
DribbleMaster should its code be released.
